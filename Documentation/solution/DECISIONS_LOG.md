...
- **Decision:** Gaurav approved the D-016 feature-bundle Read-Back (`SOLUTION_BRIEF.md` §"Phase 0 Intake — D-016 Feature Bundle") with the following resolutions to its nine named ambiguities and the architectural conflict:
  ...
  6. **Instrument drift is disclosed, not silently absorbed.** If an instrument referenced by an existing ledger later changes or is removed from the library, the ledger keeps its original data untouched and displays a prominent warning (bold, red-toned) describing what changed since the ledger was created. Never delete or silently reconcile a ledger's historical reference. **Correction, 2026-09-12: this guarantee is not in force.** Instrument drift detection was drafted as its own chunk in the D-016 Slice 5 plan and withdrawn during the erd-gate review (2026-08-24) because its `instruments.updated_at` seed-upsert semantics were an unresolved load-bearing hypothesis; it has not been rebuilt since. Nothing in the schema currently prevents a ledger's historical holding reference from dangling, and no warning fires — the doc has read as done for about three weeks while the feature does not exist. Stays open until instrument drift detection is scoped and built as its own chunk, per the original withdrawal note in `IMPLEMENTATION_PLAN.md`.
...
- **Follow-up, 2026-08-26: Chunk 6 attempted against the live site. Partially satisfied, real limit hit, not waived.** This session's Chrome extension has a hard resize floor: `window.innerWidth` will not go below about 630px no matter what width is requested (390px and even 250px both floor at 630px), most likely because this browser's toolbar and extension icons need that much horizontal room. Two workarounds were tried and both failed cleanly: an iframe sized to 390x844 was blocked by the site's own CSP `frame-ancestors` (expected, correct security behavior, not a bug), and setting CSS `zoom` on the page does not change `window.innerWidth` or `matchMedia` results, so it cannot fool a real Tailwind breakpoint. Gaurav chose to proceed at the ~630px floor rather than wait. All four live screens (Landing, Explore, Dashboard, Portfolio/Holdings) were walked signed out and signed in, scrolled top to bottom, with the console watched for errors: all four render clean at ~630px, no overflow, no clipped text, no console errors. **The one thing this did not and could not test is the exact failure class from the ledger slice's rehearsal** (a `sm:grid-cols-3` class firing wrong at exactly 390px, since this project redefines `sm` to that width) — that requires a genuine 390px viewport, which this tooling cannot produce. A real device or Chrome DevTools' device toolbar, run outside this session, is the only way to close that specific gap.

---

## D-023 — The D-021 follow-on: detail-page CTA first, Current-only writes, vault state resolved inside the sheet (2026-08-26)

- **Date:** 2026-08-26
- **Phase:** 4 Build, D-021 follow-on PR (add-from-library). Plan: `Documentation/plan/IMPLEMENTATION_PLAN.md`, section "D-021 Follow-on — Add-from-library".
- **Decision:** Build the add-from-library flow as one chunk on a fresh branch `explore-add-holding`, cut off `origin/main` @ `5217b99`, under five rulings by Gaurav:
  1. **The instrument-detail "Record this in my plan" CTA is built first, and Explore's "+ Add" is built against it** — restoring the ordering the D-016 Slice 5 plan assumed but could not have, since that CTA was specced in v1 (`COPY_DECK.md:247`, `WIREFRAMES.md:44`, `SPEC.md` §4) and never implemented. Both entry points open the same `AddHoldingSheet`, which is a second and third caller of the existing `HoldingForm`, not a new form and not a new endpoint.
  2. **Both entry points write to the baseline "Current" ledger only.** `createHolding` and `listHoldings` are called with no `ledgerId`, so a holding added from a public browse surface lands in Current, and the card's "In ledger" state is Current-scoped. There is no ledger picker on Explore.
  3. **A signed-in user with a locked vault gets an inline unlock inside the sheet, not a redirect.** They unlock in place and fall through to the prefilled form without losing their position in the library.
  4. **A signed-out visitor sees the "+ Add" button and gets an inline sign-in prompt on tap** — the same pattern as the locked case, rather than hiding the affordance. Hiding it was the alternative considered: visually cleaner, but it makes the feature invisible to exactly the audience `/explore` exists to serve.
  5. **The quarantined branch is a reference only.** `d016-slice5-chunk4-quarantine` @ `cc32697` stays orphaned as D-021 directed. Hunks are lifted from it by hand where they still apply; `explore-add-holding` is cut fresh from `origin/main` and is **not** a rebase of it.
- **Why the quarantined code could not simply be revived, despite near-zero drift.** Read directly `[P]`: comparing that branch's parent (`24817d1`) to `origin/main`, `holding-form.tsx` and `analytics.ts` are unchanged and `LibrarySection.tsx` received only the shipped retint, so the merge would have been almost mechanical. Two substantive errors are what rule it out, and neither has anything to do with the mint redesign:
  - **It gated on `<SignedIn>`, which is a Clerk session, not vault readiness.** `/explore/*` is routed deliberately outside `HouseholdGate`, and both `listHoldings` and `createHolding` call `openVault()` which calls `requireVault()`, which throws `VaultLockedError` when locked (`src/lib/holdings-api.ts:147,163`, `src/lib/crypto/key-store.ts:148`). A signed-in user with a locked vault would have seen a live "+ Add" button, filled in the whole form, and received a generic error on submit. The same swallowed failure made the "already held" derivation silently return nothing. The sheet now resolves `resolveVaultState` itself on open — and on open only, so a browsing stranger triggers no authenticated call — with a distinct inline rendering for every state that gate already names.
  - **It predates the D-016 ledger props.** `HoldingForm` gained `ledgerId` / `ledgerName` after that branch was cut, so the quarantined code passes neither. Landing in Current happens to be the right answer (ruling 2 above), but it would have been inherited by accident rather than chosen; the new code leaves `ledgerId` unpassed with a comment stating why.
- **Rejected alternative(s):** (a) Rebase `d016-slice5-chunk4-quarantine` onto `main` and ship it — rejected: it would carry Chunk 1 token-cutover noise for no gain, and the two errors above need rewriting regardless, so the rebase buys nothing a hand-lift does not. (b) Build Explore's "+ Add" alone and leave the detail-page CTA unbuilt — rejected as the sequencing D-021 explicitly flagged: it would build the second entry point against a first path that still does not exist, and would leave the D-016 Slice 5 plan's own acceptance criterion ("the existing detail-page flow's own test file passes unmodified") permanently unsatisfiable. (c) Hide the "+ Add" button from signed-out visitors — rejected, see ruling 4. (d) Redirect a locked-vault user to the unlock screen — rejected, see ruling 3: it loses their place in the library for a state that is recoverable in two fields.
- **Consequence, accepted:** this chunk restructures the Explore instrument card into a flex row containing a real button (a button cannot nest inside an anchor, so the card's outer `Link` becomes a `div` with the link and the button as siblings). That is **the exact failure class D-022 is still open on** — a layout class firing wrong at exactly 390px, this project's redefined `sm` breakpoint. Step A9 of the plan owns that verification and **will not close in this session's tooling**: the Chrome extension floors `window.innerWidth` at about 630px, as D-022's 2026-08-26 addendum records. A real device or Chrome DevTools' device toolbar is required before promotion. This chunk therefore inherits, and slightly widens, an already-outstanding gate rather than clearing it.
- **Also settled here, so it is not re-litigated during the build:** `explore_holding_added` fires **only** from the Explore card path, never from the detail-page CTA. `METRICS_PLAN.md:214` defines it as the list-level entry point, expressly "distinct from the existing detail-page 'Record this in my plan' flow". The detail path fires only the `holding_created` event `HoldingForm` already emits. Neither `METRICS_PLAN.md` nor `COPY_DECK.md` needs an edit for this chunk — both already carry what it builds.
- **Revisit if:** a ledger picker is ever wanted on Explore (ruling 2 would need reopening), or if `Unlock.tsx` turns out to resist embedding cleanly in the sheet — step A3 is scoped to decide between an `embedded` variant on `Unlock` and a thin inline form, and anything larger than that routes back rather than being absorbed.
- **Follow-up, 2026-08-26: Built, all five rulings implemented as written.** Both entry points share one `AddHoldingSheet`, both write to Current with no `ledgerId`, the locked vault gets an inline unlock, and the signed-out visitor sees the button and gets an inline sign-in prompt. The quarantine was used as a reference only, hunks were hand-lifted, and `explore-add-holding` was never rebased on top of it.
  Two design choices settled during the build that the plan left open:
  - `AddHoldingSheet` is a **controlled** component, so each entry point owns its own trigger. This keeps the held-set in `LibrarySection` and the CTA string in `InstrumentDetail`, matching the plan's ownership list.
  - Step A3's open question resolved to the **`embedded` variant on `Unlock.tsx`**, not a thin inline form. `Unlock` owns the deliberate one-message-per-method failure behaviour that must not be duplicated and allowed to diverge. The change is presentational only, and all 15 pre-existing `Unlock` tests pass unmodified. The plan's flagged risk, that `Unlock` might resist embedding and need routing back, did not materialise.
  The consequence this entry already predicted came true and is unchanged: **A9 did not close**, so this branch is not promotable.
- **Superseded same day: Gaurav explicitly overrode the A9 gate and instructed merge and push to `main` anyway**, the same override pattern as D-022 on 2026-08-25. He was told plainly, before deciding, that A9 (the 390px browser check) had not run and that the branch was not promotable under its own gate. This is a deliberate, informed override, not a resolution of the finding above. The true 390px breakpoint condition remains unverified on this code, same as it remains unverified on the rest of the live site under D-022. Both gaps now stack on the same production deploy.

---

## D-024: AI goal planner and counsel cards, deterministic engine first, LLM frames only (2026-09-07)

- **Date:** 2026-09-07
- **Phase:** Pre-Phase-0 scoping for public-showcase backlog item (2), the AI counsel and goal-planner layer first approved in D-016 and shaped in D-017/D-018.
- **Provenance, stated plainly so no future reader mistakes it for the usual process:** this decision was **not** produced by a live Phase 1 Solution Stage interview with Gaurav, which is this project's standing pattern for every feature since D-018. Gaurav explicitly authorised skipping the live interview for this item and for D-025, and substituting a synthesis of two agent runs: a **Fable 5.1 product brainstorm** and an **OpenAI Codex (Sol, high effort) engineering critique**, both run 2026-09-07 against the repository as it stands. Everything below is agent-brainstormed and synthesised, not Gaurav-answered. The open questions at the end are the parts that still need his own words before Phase 2 design can lock anything.

### Decision

1. **A deterministic client-side projection engine ships first, and stands alone.** Compound-growth maths, seeded real rates for the 6 instruments that have them, editable long-run assumptions for the other 24, and a "See the maths" panel the user can audit. This is the D-018 projections goal (feature 6), which is **not built yet**. It is a prerequisite for the AI layer, not a sibling of it. If the AI layer is never built, the engine is still a shippable, portfolio-worthy feature on its own.
2. **The LLM does zero arithmetic.** Its structured-output schema permits only instrument slugs, allocation weights in percent, and reasoning text. Every number the user sees comes from the deterministic engine. Both agents reached this independently, so it is recorded as settled rather than as one agent's preference.
3. **Entry point is a third choice in the existing "+ New" ledger modal ("Plan toward a goal"), not a standalone page.** Rejected: a separate "Strategy Lab" surface, consistent with D-018 Q1 which already rejected the same shape.
4. **Counsel cards ship in the same MVP, on demand only.** A "Review this ledger" button on any ledger, never proactive, never a background call. Apply or Dismiss, per the standing rule in D-017 item 5 and D-018 item 3.
5. **The schema enum is the enforcement mechanism for two existing hard constraints.** Output may name only library slugs, never product names, which mechanically excludes any embedded-insurance product (a standing project rule) and keeps the "education, not advice" line, since the model literally cannot emit a purchasable product name. Vocabulary is "sample", "illustration", "one way to think about it", never "recommend", "should", or "best".
6. **Cost control is four layers, not one.** The existing per-household cap (D-016), a **new global monthly circuit-breaker row in Neon** (the literal enforcement of the ₹0 ceiling, and the only layer that survives a cold start, unlike `rate-limit.ts` which is in-process), an Anthropic console spend limit as hard backstop, and payload minimisation (percentages and rounded totals only, never member names, nominees, or exact rupee amounts).
7. **Build order:** (a) deterministic engine alone, (b) goal draft through the new modal option and one proxy call, (c) counsel cards on the same proxy with a second schema.
8. **Explicitly deferred to v2:** the tune/edit flow, multiple goals per ledger, step-up SIP, tax-adjusted returns, Monte Carlo, streaming responses, and per-user caps.

### Three items the critique surfaced that this decision resolves rather than drops

**(a) D-017's "never at rest" claim is overstated and must be corrected before this ships.** D-017 item 1 states the server "sees plaintext in flight for the duration of one request only, never at rest." That is true of Vittam's own infrastructure and false end to end as worded: Anthropic's standard API retains inputs and outputs for up to 30 days absent a zero-retention agreement. The corrected claim is: **Vittam's database does not store AI-request plaintext; the AI provider may retain it under its own API policy.**

This is a copy change, not just a decision-log change, and it is **flagged here rather than written here**, deliberately:

  | Location | Required correction |
  |---|---|
  | The `/privacy` page's AI-exception section (added per D-018 Q7, which put the section on both `/privacy` and `/why`) | Replace any "never at rest" or equivalent absolute phrasing with the corrected two-clause claim above, naming Anthropic as the provider and its own retention policy as the governing one |
  | The `/why` page's matching section (same D-018 Q7 ruling) | Same correction, same wording, so the two pages cannot drift |
  | `src/lib/privacy-note.ts` | Check for and correct the same claim if it repeats there. This file is already on the standing list of pre-2026-08-05 copy carrying em-dashes, so it needs a copy pass regardless |
  | `Documentation/design/COPY_DECK.md` | The source-of-truth copy entry for both sections above, corrected in the same commit so the deck and the rendered pages cannot disagree |

  The per-transmission consent step stays as designed: the user is told, at the moment of the call, that this specific request leaves the encrypted boundary.

**(b) The cap counters must be reserved atomically, before the Anthropic call. Hard requirement for the Phase 3 plan.** `ai_plans_created` and `ai_edits_used` cannot be read, checked, then incremented after a successful response. Concurrent tabs, a double-tap, or a client retry all blow straight past the cap under that shape, and the cap is the entire cost-control mechanism (D-016 says so in as many words). The required shape is a **reservation taken before the outbound call**: either a single conditional `UPDATE ... SET counter = counter + 1 WHERE counter < cap` that succeeds or affects zero rows, or a reservation row carrying a **unique constraint on a client-supplied idempotency key**. Neon over `neon-http` has no transactions, so this must be expressible as one statement. Whether a failed call releases its reservation is a Phase 2 design question, not a reason to weaken the requirement.

  **Resolved 2026-09-07 by Gaurav: a failed call does NOT refund its reservation.** A client-reported failure counts as a used reservation regardless of cause. Refunding would make the cap gameable, because "the call failed" is a claim the client makes and a client that makes it every time gets unlimited calls. The counter is the entire cost-control mechanism (D-016 says so in as many words), so it has to be un-gameable rather than fair. `ai_call_reservations.status` moves to `failed` and the counter stays consumed. The cost is real and is a copy problem, not a schema problem: the user is told before the call, at the moment of consent, that a failed attempt still counts. This confirms the strict direction `DATA_MODEL.md`'s Stage 0 section already drafted, and that section is now the resolved record rather than a proposal.

**(c) Prompt caching is a live conflict between the two agents and is NOT resolved here.** See the open questions below.

### Rejected alternatives

- **A standalone goal-planner page.** Rejected for the same reason D-018 Q1 rejected a "Strategy Lab": it splits the ledger surface in two.
- **The LLM computing any number.** Rejected by both agents. It puts arithmetic the user cannot audit behind a non-deterministic call, on a product whose regulatory line is "education, not advice".
- **Relying on `rate-limit.ts` as the ₹0 enforcement.** Rejected: it is in-process and resets on every cold start, so it bounds a burst and bounds nothing monthly.
- **Sending exact rupee amounts, member names, or nominees in the payload.** Rejected: percentages and rounded totals answer every question the model is being asked.
- **Proactive counsel cards.** Rejected: every proactive card is an uncapped API call for a user who did not ask.

### Open questions that need Gaurav's own answer before Phase 2 design starts

1. **The shared Anthropic API key, reopened not resolved.** D-018's 2026-08-17 addendum records Gaurav choosing to reuse `group-travel-pwa/backend/ANTHROPIC_API_KEY` from gopass rather than mint a project-scoped key, with the shared-billing risk noted as "worth a line in that build slice's own risk note if it matters later, not a blocker now." This is that build slice, and Codex independently re-raised the same risk without knowing D-018 had already flagged it. The tradeoff, plainly: **reuse** is zero extra setup and one credential to manage, but usage and billing are not isolated, a runaway loop in either project drains the other, and a rotation forced by one project breaks the other. **Minting a dedicated key** with its own capped prepaid credit and auto-reload disabled bounds the blast radius by construction, at the cost of one trip to Gaurav's Anthropic console, which Claude cannot do for him. Codex recommends the dedicated key. **Not decided here.**

   **Resolved 2026-09-07 by Gaurav: reuse `group-travel-pwa/backend/ANTHROPIC_API_KEY`. No dedicated key will be minted.** This diverges from both Codex's recommendation and the implicit direction of D-018's own note, and it is recorded as a deliberate accepted risk, not as agreement that it is the safer option. It is not the safer option. What is being accepted, stated plainly so no later reader has to reconstruct it:
   - **Shared blast radius.** A runaway loop, a leaked key, or a compromised deploy in either Vittam or group-travel-pwa exposes and drains the other. The two projects have no relationship beyond this credential.
   - **No per-project attribution.** Anthropic console spend and usage cannot be split between the two projects. The "roughly ₹2,800 lifetime at 200 households" figure in D-018 becomes unverifiable from billing alone, so the cost model is only ever confirmable from Vittam's own cap counters.
   - **Coupled rotation.** A rotation forced by either project silently breaks the other until both Vercel environments are updated. There is no alert wired for that; the failure surfaces as AI calls returning 401 in production.
   - **The console spend limit is now shared too.** D-024 decision 6 names an Anthropic console spend limit as the hard backstop layer. That backstop now protects both projects jointly, so a group-travel-pwa incident can exhaust Vittam's backstop and vice versa.

   What buys this down: the per-household cap (D-016) and the new global monthly circuit-breaker row (D-024 decision 6) are both enforced inside Vittam and are unaffected by the shared key. They bound Vittam's own contribution regardless of what the other project does. They do not bound what the other project does to Vittam.

   **Revisit trigger for this specific choice:** the first time either project's spend cannot be explained from its own counters, or the first rotation of this key for any reason. At that point minting a dedicated key costs one trip to the console and removes all four items above.
2. **Prompt caching at the Anthropic layer: the two agents directly disagree.** The brainstorm recommends caching the system prompt and instrument library, roughly halving per-call cost from about ₹2.40 to ₹1.20. The critique says the proxy should use no prompt caching, no Files API, no queues, and no retries at the Anthropic layer, on the grounds that each is another place plaintext can persist. Both positions are coherent; they optimise different things, cost against retention surface. Since the cap already bounds worst-case spend to roughly ₹14 per household, the absolute saving is small, which argues for the critique's position, but this is Gaurav's call and is **left open**.

   **Resolved 2026-09-07 by Gaurav: no prompt caching. Codex's position is adopted.** The proxy sends a plain, uncached request every time. The reasoning on file, in the order it carries weight:
   - **Retention surface is the thing being minimised, and caching adds one.** A cache write is a second place the request content persists at the provider, on the provider's own lifetime rather than the request's. D-024 item (a) already forces Vittam to correct its "never at rest" claim to acknowledge Anthropic's own retention policy. Adding a cache widens exactly the surface that correction is about, for a saving that does not need to be won.
   - **The cap already bounds the worst case, so caching's savings do not buy much.** Per D-018 §5 the per-household worst case at the cap is about ₹14. Halving per-call cost from roughly ₹2.40 to ₹1.20 moves that to about ₹7. Saving ₹7 per household at the absolute ceiling is not worth a new persistence surface on a product whose stated differentiator is that the server cannot read your data.
   - **It stays consistent with the rest of the proxy's shape.** D-024 decision 6's payload minimisation and the D-024 route-hardening list (no request or response logging, no Sentry body capture, `Cache-Control: no-store`) all pull the same direction: nothing about this call is kept anywhere it does not have to be. Caching would be the one exception in that set.
   - This also settles, in the same direction and for the same reason, the Files API, queues, and retries at the Anthropic layer: none of them are used. A failed call fails to the user, who may ask again.

   **Revisit if:** the cap moves substantially upward (per-user caps, a paid tier) so that per-call cost starts to dominate, or Anthropic zero-retention terms are in place for this account, at which point the retention argument weakens and the cost argument is the only one left.
3. **The `goals` table cannot ship in its current shape.** It is plaintext, which D-014 forbids for household data. The brainstorm's proposal is to move goal data inside the ledger's existing encrypted envelope rather than give it its own table. Needs confirming before Phase 2 Stage 0 touches the data model.

   **Resolved 2026-09-07 by Gaurav: goal data lives inside the existing ledger envelope.** The sealed payload widens from `{ name }` to `{ name, goal }`. No new table, no new column, no migration, and no server change, because the server never sees the payload shape at all. A goal has exactly one ledger and a ledger has at most one goal, so a separate table would buy a join and a second AAD binding for nothing. **The plaintext `goals` table stays in the schema, unused, exactly as `analytics_events` does under D-012.** Dropping it is its own migration and its own decision, and folding one into this feature would mix unrelated work. This confirms `DATA_MODEL.md`'s Stage 0 draft for D-024 rather than changing it.
4. **The global monthly AI cap's actual number.** Carried as an open question in `SPEC.md` §G8.2 rather than here: `ai_global_usage.cap_calls` is specced, the value is a cost judgment Gaurav owns.

   **Resolved 2026-09-07 by Gaurav: 50 calls per month.** `ai_global_usage.cap_calls = 50`, seeded from a server constant when the month's row is created lazily. At the uncached per-call cost already on record in D-018 §5 (roughly ₹2.40, and uncached is now the settled shape per open question 2 above), 50 calls bounds worst-case monthly AI spend near ₹120. That is the literal enforcement of the ₹0-ish ceiling D-024 decision 6 names, and it is the only cost layer that survives a cold start. The value is stored per row, so raising the cap later does not retroactively rewrite a past month's record. **Revisit if:** real usage regularly trips the breaker before month end, which would mean the cap is bounding legitimate use rather than runaway cost.
5. **Whether the goal horizon control keeps both presets and a free field.** Carried as an open question in `SPEC.md` §G8.3.

   **Resolved 2026-09-07 by Gaurav: both.** Preset chips at 5, 10, 15 and 20 years, plus a free-entry numeric field for an exact year count. This matches how the projection panel already lets users edit assumptions rather than choose from a fixed list, so the two controls in the same feature behave consistently. Presets carry the common case; the field carries a real goal year, which is the case a preset cannot express.

### Ship traps inherited from this project's own history, to be carried into the Phase 3 plan

- Verify migrations against the configured Neon database via `npm run db:probe`, never against `drizzle/migrations/meta/_journal.json` (the 2026-08-04 lesson).
- Verify CSP and auth behaviour on a live Vercel deploy, not locally (the 2026-08-05 Turnstile lesson). **Never add the Anthropic host to the browser CSP**: the browser never calls Anthropic, the proxy does.
- Render at a true 390px before shipping. D-022 and D-023 are both still open on exactly this, stacked on one deploy.
- Route hardening for the proxy: auth before any processing, strict body and token limits, schema allowlisting in both directions, no request or response logging, no Sentry body capture, `Cache-Control: no-store`.

- **Revisit if:** the deterministic engine alone satisfies Gaurav once he sees it (then the AI layer may not be worth building at all, which is a legitimate outcome of building the engine first), or the circuit-breaker row proves to need a second write path that the `neon-http` no-transaction constraint cannot express.

---

## D-025: Bulk holdings import from Excel, import only, prefilled per-member tabs, parsed rows never persisted (2026-09-07)

- **Date:** 2026-09-07
- **Phase:** Pre-Phase-0 scoping for public-showcase backlog item (3), bulk holdings Excel import, first approved in D-016 item 3 and given its PII ruling in D-017 item 9.
- **Provenance:** same as D-024. **Not a live Phase 1 interview.** Synthesised from a Fable 5.1 product brainstorm and an OpenAI Codex (Sol, high effort) engineering critique, both run 2026-09-07, with Gaurav's explicit authorisation to substitute this for the usual interview. The open questions at the end still need his answer.

### Decision

1. **Import only for v1. Export is a separate encryption-boundary decision and is not a side effect of import.** The template shape is designed so a future export needs two hidden columns added, not a redesign.
2. **Template shape: one tab per household member, 30 prefilled instrument rows per tab, grouped by asset class.** No free-text instrument entry. A second holding of the same instrument is handled by duplicating its prefilled row. Columns: hidden slug, asset class (display only), instrument name, amount invested, current value, units, monthly SIP, start date, maturity date, nominee, emergency-fund flag, notes.
3. **Kind-awareness is shading plus cell comments, never a hard block.** This matches how the in-app form already behaves: it accepts all fields on all instruments.
4. **The target ledger is chosen in the app, never encoded in the file.** Whichever ledger tab is active receives the import, so a stale download cannot write into the wrong ledger.
5. **Parsed rows are memory-only until sealed.** No parsed-but-unsealed row may touch IndexedDB, localStorage, sessionStorage, or any persisted cache.
6. **Review screen before commit, partial commit is the default.** Rows land in four buckets (Ready, Needs attention, Possible duplicate, Skipped) with plain-language per-row reasons, and the primary action commits the clean ones ("Add 11 holdings to Current"). A "download the rows that need attention" file closes the fix-and-retry loop.
7. **Fuzzy instrument matching may only ever suggest a candidate for explicit confirmation. Never auto-resolve.** Both agents said this independently.
8. **Two India-specific parsing traps get tests, named now so they are not rediscovered late.** Excel date serials must be formatted from local date parts, never through `toISOString()`, which shifts a day under the IST offset. Lakh-grouped numbers ("1,50,000") must parse as 150000, and shorthand ("1.5L") is **rejected with a clear message in v1**, never guessed.
9. **Not re-litigated, already settled:** the template ships prefilled with member names and that PII exposure is accepted and disclosed (D-016 item 3, D-017 item 9); foreign CSVs and bank statements are out of scope; `MAX_LEDGER_HOLDINGS` is the row cap; the commit reuses `memberScopedCreateSchema` and the existing ledger-ownership and member-tenancy checks. Events `bulk_import_template_downloaded`, `bulk_import_completed{rows_clean, rows_rejected}` and `pii_disclosure_shown` are already defined in `METRICS_PLAN.md`.

### Hard requirements for the Phase 3 plan: the plaintext-leak surfaces

Named specifically because this project has already shipped one PWA offline-scope defect (the D-013/B-005 class, and again the `virtual:pwa-register` gap closed 2026-08-06). These are concrete risks here, not hypothetical.

- **The service worker must not cache the import screen's row data or any `/api/*` request or response body.** vite-plugin-pwa's runtime caching config needs an explicit exclusion, and a test pinning it, in the same style as `pwa-registration.config.test.ts`.
- **The parser chunk must be explicitly added to the precache list**, or the import screen breaks offline. This is the mirror image of the point above and both must be handled in the same pass.
- **Sentry breadcrumbs, session replay, and console output must be audited and scrubbed of row data.** Per-cell validation messages must name the column and the reason, never echo the offending value.
- **No clipboard-based import path** in v1.
- **Browser-extension file access is outside Vittam's trust boundary.** Disclose it in the import screen's copy; do not attempt to engineer around it.

### Open questions that need Gaurav's own answer before Phase 2 design starts

1. **Batch endpoint against serial per-row calls. Codex called this a real scope decision, not a settled fact, and it changes the size of the Phase 3 plan.** The brainstorm assumed a single atomic `POST /api/holdings-batch` (note: **single-segment routing only**, so `/api/holdings/batch` is impossible on this project's Vercel config, a limitation already recorded in `app/CLAUDE.md`). Codex agreed on the endpoint but flagged it as scope Gaurav should confirm, and offered serial per-row calls with a progress bar as the v1 alternative if imports are typically tens of rows rather than hundreds. **Recommendation: build the batch endpoint.** Reasoning: the atomic multi-row insert path already exists in spirit at `POST /api/ledgers`, which accepts multiple browser-sealed holdings in one insert, so this extends a proven shape rather than inventing one; serial calls turn partial commit (decision 6 above) into a client-side loop with its own failure and resume states, which is more moving parts, not fewer; and `neon-http` has no transactions, so a serial path cannot be made atomic at all, only "mostly succeeded". **Flagged for confirmation because it materially enlarges the plan.**

   **Resolved 2026-09-07 by Gaurav: build the atomic batch endpoint `POST /api/holdings-batch`. Serial per-row calls are rejected.** The plan is accepted at its larger size. The reasoning already on file is the reasoning adopted, restated so it is not re-derived:
   - **It extends a proven shape rather than inventing one.** `POST /api/ledgers` already accepts an array of browser-sealed holdings (`createLedgerSchema.holdings: z.array(memberScopedCreateSchema).max(MAX_LEDGER_HOLDINGS)`, `server/lib/ledgers.ts`) and inserts them in one statement. The batch endpoint is that same body shape pointed at an existing ledger instead of a new one.
   - **`neon-http` has no transactions, so a serial path can never be atomic.** It can only ever be "mostly succeeded", which turns every failure into a resume-state problem the client has to own.
   - **Serial calls make partial commit worse, not simpler.** Decision 6 above makes partial commit the default. Under a serial loop that becomes a client-side progress machine with its own failure, retry, and abandonment states. Under one batch insert it is a single server round trip over an already-filtered set of clean rows.
   - **Route path is forced.** `/api/holdings/batch` is impossible on this project's Vercel config (single path segment only, `app/CLAUDE.md` 2026-07-11 note). `/api/holdings-batch` is the shape that works, and it is a new top-level Hono route mount, not a sub-path of the existing holdings router.

   **Revisit if:** real imports run to hundreds of rows rather than tens, at which point batch sizing and bounded concurrency become a genuine design problem, per this decision's own Revisit-if clause.
2. **Spreadsheet dropdown validation, yes or no. This is the single highest-leverage open product question and it changes the library choice.** If Gaurav wants true Excel data-validation dropdowns on the instrument column, the library must be **ExcelJS** (heavier, more slowly maintained) and the template design changes. If not, **SheetJS Community Edition** stands, pinned to the vendor's own tarball URL rather than the frozen npm registry copy, loaded via dynamic import so the main bundle and the 2s load target are untouched, and the prefilled-row design in decision 2 works as written. **The brainstorm's recommendation, and the one this record assumes: no dropdowns, SheetJS, prefilled per-member tabs.** Needs Gaurav's confirmation before Phase 2 locks the template shape. Note that dropdowns buy less than they appear to: Excel data validation is bypassable by paste and behaves inconsistently across Excel, Sheets, and LibreOffice, so the importer must re-validate every instrument reference regardless of what the spreadsheet enforces.

   **Resolved 2026-09-07 by Gaurav: no dropdowns. Prefilled rows stand.** Three things lock at once on this answer, and open question 3 below is settled by it rather than separately:
   - **Library locks to SheetJS Community Edition**, pinned to the vendor's own tarball URL rather than the frozen npm registry copy, loaded via dynamic import so the main bundle and the 2s load target are untouched. ExcelJS is rejected: its only advantage was true Excel data validation, which is now not wanted. `read-excel-file` / `write-excel-file` (Codex's independent suggestion) is also rejected, on the merits Fable named and Codex did not address: it cannot produce hidden sheets or cell comments, and the template design depends on both (the hidden slug column, and kind-awareness delivered as cell comments per decision 3).
   - **Template shape locks to decision 2 exactly as written:** one tab per household member, all 30 library instruments prefilled per tab, grouped by asset class, hidden slug column, no free-text instrument entry, a second holding of the same instrument handled by duplicating its prefilled row.
   - **Nothing about the importer's validation changes**, because it never depended on the spreadsheet enforcing anything. Every instrument reference is re-validated against the library on parse regardless, exactly as this open question already noted.

   **Revisit if:** real users are observed typing instrument names into the name column instead of using the prefilled rows, which would mean the prefill is not discoverable and some in-sheet affordance is needed after all.
3. **The two agents named different libraries, and this is not silently resolved.** Fable recommends SheetJS Community Edition; Codex independently recommended `read-excel-file`. Fable's brainstorm considered and rejected `read-excel-file` on the grounds that it cannot produce hidden sheets or cell comments, both of which the template design in decision 2 relies on. Codex did not address that constraint, which suggests Fable's rejection stands on the merits, but Codex's own contribution here (the precache gap) applies to whichever parser is chosen. **Recorded as a disagreement for Gaurav to settle, not resolved here.**

   **Resolved 2026-09-07 by Gaurav's answer to open question 2: SheetJS Community Edition wins, `read-excel-file` is rejected.** See the resolution note under open question 2 above for the full reasoning; the deciding constraint is the hidden-sheet and cell-comment requirement, which survives Gaurav's no-dropdowns answer intact. Codex's own contribution on this point, the service-worker precache gap, is unaffected by the library choice and remains a hard requirement of the Phase 3 plan.

### Rejected alternatives

- **A blank grid template.** Rejected in D-016 item 3 already; restated so it is not re-proposed.
- **`read-excel-file` / `write-excel-file`.** Rejected by the brainstorm for the hidden-sheet and cell-comment gap. See open question 3, since Codex disagreed.
- **Any hosted parsing API.** Rejected: it breaks both the ₹0 ceiling and the encryption boundary in one move.
- **Guessing at shorthand amounts ("1.5L").** Rejected: a wrong guess about money is worse than a clear rejection message.

### Verification gap, flagged now in the same class as the 390px gate

Vitest will prove the parser against fixture files the generator itself wrote, which is a closed loop: it cannot catch a difference between what this code writes and what real Excel, Google Sheets, or LibreOffice writes on save. **One manual cross-tool pass is owed before promotion**, and it should be written into the Phase 3 plan as its own gate rather than left as an intention, given this project's history of gates being overridden at merge time (D-022, D-023).
