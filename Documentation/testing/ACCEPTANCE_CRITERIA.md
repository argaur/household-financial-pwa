...
**Live verification: steps 1–7 PASSED 2026-07-28**, step 8 blocked, remainder outstanding. Verified so far: step 1's four cards render in the documented order (Your household / Family members / Protection / Account); step 2's Edit swaps in an editable field with Save changes + Cancel; step 3's rename showed the new name with no page reload; step 4's member row (not Remove) opens "Update family member" pre-filled with name, relationship, DOB and the blank optional risk profile; step 5's rename updated both the member row *and* the Protection card's group heading; step 6 added "Test Member Two" (Spouse) as a second row; step 7's confirm dialog carried the documented copy verbatim ("Remove Test Member Two? This will remove … This cannot be undone.") and "Keep them" closed it with the member retained.

**Step 8 PASSED** — performed by hand after B-003 (the dialog refusing to reopen under automation) turned out to be automation flake, not a defect. Both members were removed successfully.

**Cascade delete verified live, incidentally.** The script does not ask for this, but removing "Renamed Member One" — who held 2 holdings and 2 protection records — left the household with **0 members, 0 holdings, 0 protection** (confirmed by querying `/api/family-members`, `/api/holdings` and `/api/protection` directly). That is the promise in step 7's dialog copy ("will delete any holdings or protection cover recorded for them") proven against real data, and it exercises the member-level cascade independently of the account-level cascade tested at step 12.

**Empty-household nudge fallback also exercised:** with zero members, `/api/dashboard` returns the `member_coverage` nudge with **no** `memberName`, and the card falls back to "Every member in your plan should have at least one investment or asset mapped." rather than rendering a sentence about an undefined person. That is the no-name branch `nudge-card.tsx` documents but which no live pass had reached before.
...
**Step 12 PASSED 2026-08-01, verified server-side.** Gaurav ran the deletion by hand. The cascade was then confirmed by querying the production database directly in the Neon console rather than trusting the UI's confirmation: households **3 → 2** (`Gaurav's Family | Renamed Household`, Sharma Family gone), family_members **5 → 3**, holdings **4 → 1**, protection **1 → 0**, instruments **30, untouched**. Exactly the rows the deleted account owned, and nothing else.

**This is the only end-to-end proof that the production Clerk webhook and its new signing secret work.** The unsigned-POST probe returns 401 either way, so it could not distinguish the new production secret from the stale development one still in Vercel earlier that day. A real `user.deleted` event firing and cascading correctly is what settles it. Had the secret been wrong, the UI would have reported success while every row survived.

**Step 11 remains owed, and needs a human to click.** The delete-account confirm dialog did not open under two synthetic clicks on `ref_31`. This is **B-003 recurring**, closed on 2026-07-28 as automation flake after Gaurav opened the same dialog by hand on the first try. Per B-003's own recorded lesson — when a Radix control refuses a synthetic click, suspect the driver and get a human to press it rather than filing a defect — no new bug is raised. Step 12 needed an explicit go-ahead regardless.
...
**Result:** Built and smoke-run live 2026-07-21 (browser-driven): `/why` renders correctly in the locked design language (DM Serif Display hero, warm card surfaces, Decision/Instead of/Why labels), the sign-in footer link resolves to `/why`, and axe-core scans of `/why` and `/explore` pass with zero violations after the contrast fix. **The authenticated dashboard was then axe-scanned on production** (once the token fixes deployed) and surfaced two real issues the public-page/localhost scans could not reach — both fixed in follow-up commits: (1) `svg-img-alt` — Recharts renders each donut sector as an unnamed `<svg>`/`<path>`; the chart is now wrapped in an `aria-hidden` div (the labelled `role="img"` parent + text legend are the accessible representation); (2) `aria-hidden-focus` — Recharts stamps each sector with `tabindex="-1"`, which an `aria-hidden` container may not contain, so the stray tabindex is stripped after render (`allocation-donut.tsx`). **Final production axe scans — all five screens 0 violations:** `/why`, `/explore`, `/dashboard`, `/portfolio`, `/profile` (wcag2a / wcag2aa / wcag21aa). Automated suite: 300 tests (5 new for `Why`), typecheck / `check_events.py` / `npm run build` all clean.
