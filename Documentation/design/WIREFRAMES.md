...
**Carried into Phase 3, unresolved by design (SPEC.md §S10) — all three now closed as of the Phase 3 plan, 2026-08-25:** font-loading strategy (resolved — Playfair Display/Jost/JetBrains Mono loaded via Google Fonts), whether the Explore "+ Add" entry point is new scope needing its own analytics event (resolved — confirmed new scope, `explore_holding_added` event added, and the interaction itself corrected during the plan's gate review from an assumed instant toggle to what the folio's own copy actually specifies: it opens the existing add-holding form, prefilled), and the standing regression-risk mitigation (recorded as Chunk 6, mandatory, in `IMPLEMENTATION_PLAN.md`'s D-016 Slice 5 plan — not resolved at Design time by decision, matching the D-016 ledger slice's own rehearsal pattern).
